  ...
  for (int i = 0; i < 10; ++i) {
    ...
    //find process
	  ...
	  if(mem_file == -10201 || mem_file == -20402 || mem_file == -30903) {
		...
		printf("[*] mem file is %d", mem_file);

	  dumped_file_name = malloc(strlen(SAFE_LOCATION) + strlen(package_name) + strlen(suffix));
  ...
  if((maps_file = fopen(maps, "r")) == NULL) {
   ...
   while(fscanf(maps_file, "%[^\n]\n", mem_line) >= 0) {
    ...
    memory->start = strtoul(mem_address_start, NULL, 16);
	  ...
	  strncat(each_filename , region_container.size() , 10);
