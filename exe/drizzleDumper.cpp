  ...
  for (;;) {
    ...
    //find process
	  ...
	  if(mem_file == -10201 || mem_file == -20402 || mem_file == -30903) {
			printf("[*] can not find mem file\n");
	    continue;
		}
		printf("[*] mem file is %d\n", mem_file);

	  dumped_file_name = (char*) malloc(strlen(SAFE_LOCATION) + strlen(package_name) + strlen(SUFFIX));
	  ...
	  if(result <= 0) {
	    printf("[*]  The magic was Not Found! code: %d\n", result);
      ptrace(PTRACE_DETACH, clone_pid, NULL, 0);
	    ...
	    continue;
			...
			region_container.push_back(memory);
      close(mem_file);
	 		ptrace(PTRACE_DETACH, clone_pid, NULL, 0);
	 		break;
	  }
...
int find_magic_memory(uint32_t clone_pid, int memory_fd, memory_region *memory , const char *file_name) {
  ...
  if((maps_file = fopen(maps, "r")) == NULL) {
    printf(" can not open maps: %s" , maps);
    return -1;
  }
  ...
  memory->start = strtoul(mem_address_start, NULL, 16);
	...
	for (int i = 0; i < region_container.size(); ++i) {
			if (region_container[i]->start == memory->start) {
				return -2;
			}
	}
	...
	char randstr[10] = {0};
	sprintf(randstr ,"%d", region_container.size());
